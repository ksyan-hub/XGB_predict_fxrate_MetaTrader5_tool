...
def train_model_low(start, end, window, tree_batch_size):
    ...
    with open(path.trained_model_predictions_low(), "w", newline = "") as predict_file:
        writer = csv.writer(predict_file)
        writer.writerow(["0","predicted","actual","se","mse"])

    with open(path.trained_model_predictions_low(), "a", newline ="") as predict_file:
        writer = csv.writer(predict_file)
        ...
        for i in range(window, len(train_df_USDJPY_5m)):
            ...
            y_buffer.append(train_df_USDJPY_5m.iloc[i]["low"])
            ...
            if booster is not None:
                ...
                se = int(((predictions - train_df_USDJPY_5m.iloc[i]["low"])**2)*prcssng_float)/prcssng_float #小数点を切り捨て、誤差を2乗
                se_buffer.append(se)
            ...
            writer.writerow([ i - (window - 1), prcssng_predictions, train_df_USDJPY_5m.iloc[i]["low"],se,mse])
            ...
            if i + 1 == len(train_df_USDJPY_5m):
                print(f"lowの{i}行目まで学習完了。{booster.num_boosted_rounds()}")
                booster.save_model(path.save_trained_model_low())
                break